...
const escapeValuesWithQuotes = (str: string): string =>
	str.match(/\$-?(\d{1,3},+)?(\d{1,3},\d{3}(\.\d)*)/g)
		? str.replace(/,\$/g, '","$')
		: str.replace(/,/g, '","');

const wrapWithQuotes = (text: csvRow) =>
	typeof text === "string" ? `"${text.replace(/"/g, '""')}"` : `"${text}"`;

const formatMixedEntry = (arr: csvRow): string =>
	arr.constructor === Array
		? arr.map(wrapWithQuotes).join()
		: wrapWithQuotes(arr);
		...
		[...value]
			.map((text: csvRow) => {
				...
				return textString.includes(",")
					? formatEntryWithCommas(textString, text)
					: wrapWithQuotes(textString);
			})
			.join(","),
	);
