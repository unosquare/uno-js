...
const formatMoney = (
	...
	const amount = !parsedMoney
		...
		: new Intl.NumberFormat(locale, { style: "currency", currency }).format(
				truncate(parsedMoney, 100000),
			);

	return showCurrency ? `${amount} ${currency}` : amount;
};

const formatDays = (stringData: string) =>
	Number(stringData) === 1 ? "1 day" : `${stringData} days`;

const formatMonths = (stringData: string) =>
	Number(stringData) === 1 ? "1 month" : `${stringData} months`;
...
export const formatter = (
	...
	return data === null
		...
		: internalFotmatter(
				String(data),
				{ keepFormat, decimals, nullValue, locale, currency, showCurrency },
				format,
			);
};
...
export const toMoney = (
	data: unknown,
	options?: { locale?: string; currency?: string; showCurrency?: boolean },
) => {
	const defaultOptions = {
		...options,
...
export const toPercentage = (data: unknown, options?: { decimals?: number }) =>
